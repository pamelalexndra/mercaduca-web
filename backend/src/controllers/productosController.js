...
export const getProductos = async (req, res) => {
    try {
        ...
        const simpleQuery = await pool.query('SELECT COUNT(*) as count FROM producto');
        console.log('Total productos:', simpleQuery.rows[0].count);

        // Obtenemos todos los parámetros de filtro
        const { ids, precio_min, precio_max, ordenar = 'fecha_desc', emprendimiento_id, limit, search } = req.query;
      ...
      WHERE p.Disponible = true`];
        ...
        if (ids) {
            const categoriasIds = ids.split(',').map(id => parseInt(id.trim())).filter(id => id > 0);
            if (categoriasIds.length > 0) {
                const placeholders = categoriasIds.map((_, i) => `$${params.length + i + 1}`).join(',');
        ...
        queryParts.push(` ORDER BY ${ordenamientos[ordenar] || ordenamientos.fecha_desc}`);
...
export const updateProductoPartial = async (req, res) => {
    try {
        ...
        const allowedFields = ['nombre', 'descripcion', 'imagen_url', 'precio_dolares', 'existencias', 'disponible', 'id_categoria'];
        const setParts = [];
        const params = [];
        let paramCount = 1;
        ...
        for (const [key, value] of Object.entries(updates)) {
            if (allowedFields.includes(key)) {
                ...
                else if (key === 'existencias' || key === 'id_categoria') params.push(parseInt(value));
