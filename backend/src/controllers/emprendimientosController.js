...
export const getEmprendimientos = async (req, res) => {
    try {
        ...
        console.log('Conexión BD:', dbCheck.rows[0].time, 'Total emprendimientos:', dbCheck.rows[0].total);
...
export const updateEmprendimientoPartial = async (req, res) => {
    try {
        ...
        const allowedFields = ['nombre', 'descripcion', 'imagen_url', 'instagram', 'disponible', 'id_categoria'];
        const setParts = [];
        const params = [];
        let paramCount = 1;
...
export const deleteEmprendimiento = async (req, res) => {
    try {
        ...
        if (parseInt(productosCheck.rows[0].count) > 0) {
            return res.status(400).json({
                error: 'No se puede eliminar el emprendimiento porque tiene productos activos. Elimine primero los productos.'
            });
        }
